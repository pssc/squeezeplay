...
static int log_logger(lua_State *L) {
	...
	luaL_getmetatable(L, "squeezeplay.log.obj");
...
int luaopen_log(lua_State *L) {
	/* methods */
	luaL_newmetatable(L, "squeezeplay.log.obj");
	...
	/* class */
	luaL_register(L, "squeezeplay.log", log_f);

	return 0;
}


int squeezeplay_log_init(lua_State *L) {
	char *log_path;

	/* configure logging */
	log_path = alloca(PATH_MAX);
	if (!squeezeplay_find_file("logconf.lua", log_path)) {
		return 0;
	}
